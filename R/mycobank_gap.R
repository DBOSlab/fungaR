...
mycobank_gap <- function(taxon,
  ...
  if (is.null(filename)) {
    filename <- paste0("mycobank_gap_", gsub("\\s+", "_", taxon))
  }
  ...
  if (html_report && nrow(result) > 0) {
    ...
    dir <- .arg_check_dir(dir)
    .funga_render_report(template = "mycobank_gap_report.Rmd",
