...
#' \code{\link{funga_mycobank_gap}}, this function does not cross-check FFB at all: it
...
#'   Defaults to \code{"funga_mycobank_records"}.
#'
#' @param filename Character. Name of the \code{.xlsx} file (without extension) to save.
#'   Defaults to \code{"funga_mycobank_records_<taxon>"}.
#'
...
#' }
#'
#' @seealso \code{\link{funga_mycobank_gap}}, \code{\link{funga_distribution_gap}}
#'
#' @author
#' Domingos Cardoso
#'
...
#' all_records <- funga_mycobank_records(taxon = "Trichoderma", rank = "genus",
...
funga_mycobank_records <- function(taxon,
                                   rank = c("species", "genus", "order"),
                                   check_locality = TRUE,
                                   require_brazil_evidence = TRUE,
                                   max_check = 200,
  ...
  if (is.null(filename)) {
    filename <- paste0("funga_mycobank_records_", gsub("\\s+", "_", taxon))
  }
  ...
  if (html_report && nrow(result) > 0) {
    ...
    dir <- .arg_check_dir(dir)
    .funga_render_report(template = "funga_mycobank_records_report.Rmd",
